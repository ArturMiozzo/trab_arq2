...
int salvaArquivo(OBJETO* ptr_objeto, char dados[], uint8_t ind_cluster_pasta){
    ...
    if(salvaObjetoNaPasta(*ptr_objeto, ind_cluster_pasta) == 0){    //Se o objeto já existe, a função é encerrada
        return 0;
    }


    //Começa a etapa de armazenar os dados
    if(ptr_objeto->tamanho <= sizeof(CLUSTER)){                 //Os dados cabem em um único cluster
